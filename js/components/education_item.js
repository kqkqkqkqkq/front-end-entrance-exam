import { createEditableText } from './editable.js'

export function createEducationItem(data, isFavorite = false) {
  const item = document.createElement('div')
  item.classList.add('education-item')

  const topRow = document.createElement('div')
  topRow.classList.add('education-top-row')

  const time = createEditableText(data.time, 'education-time')

  topRow.appendChild(time)

  const badge = document.createElement('span')
  badge.innerHTML =
    `
    <svg viewBox="0 0 11 11" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M2.75 0.807373C1.99375 0.807373 1.32 1.12388 0.81125 1.61929C0.31625 2.1147 0 2.78901 0 3.55965C0 4.31653 0.31625 4.99083 0.81125 5.5L5.5 10.1926L10.1887 5.5C10.6838 5.0046 11 4.33029 11 3.55965C11 2.80277 10.6838 2.12847 10.1887 1.61929C9.69375 1.12388 9.02 0.807373 8.25 0.807373C7.49375 0.807373 6.82 1.12388 6.31125 1.61929C5.81625 2.1147 5.5 2.78901 5.5 3.55965C5.5 2.80277 5.18375 2.12847 4.68875 1.61929C4.19375 1.12388 3.52 0.807373 2.75 0.807373Z" fill="#F6ED1E"/>
</svg>
    `
  badge.classList.add('education-badge')
  topRow.appendChild(badge)

  if (isFavorite) {
    badge.style.display = 'inline-block'
  } else {
    badge.style.display = 'none'
  }

  const titleContainer = document.createElement('div')
  titleContainer.classList.add('education-title-container')

  const title = createEditableText(data.title, 'education-title')
  const tags = createEditableText(data.tags, 'education-tags')

  titleContainer.appendChild(title)
  titleContainer.appendChild(tags)

  const info = createEditableText(data.info, 'education-info')

  item.appendChild(topRow)
  item.appendChild(titleContainer)
  item.appendChild(info)

  return item
}
